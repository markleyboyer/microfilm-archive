...
function loadFiles(cell, g) {
  ...
  if (viewMode === 'thumb') {
    ...
    // ── Thumbnails — built in batches to keep the page responsive ──────────
    ...
    const BATCH = 20;
    let bi = 0;
    function addThumbBatch() {
      const frag = document.createDocumentFragment();
      for (let end = Math.min(bi + BATCH, g.files.length); bi < end; bi++) {
        const f   = g.files[bi];
        const idx = bi;
      ...
      if (bi < g.files.length) requestAnimationFrame(addThumbBatch);
    }
    addThumbBatch();
    ...
    function addListBatch() {
      ...
      wrap.appendChild(frag);
      if (bi < g.files.length) requestAnimationFrame(addListBatch);
    }
    addListBatch();
  }
}
